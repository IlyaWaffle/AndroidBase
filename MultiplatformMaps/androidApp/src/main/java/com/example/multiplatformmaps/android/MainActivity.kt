...
class MainActivity : AppCompatActivity(), UserLocationObjectListener {
    ...
    private var tmpLocation: Point? = null
    ...
    fun onMusicButtonClick(view: View){
        if (sound?.isPlaying == true){
            val musicButton = findViewById<Button>(R.id.musicButton)
            musicButton.text = "Play Audio"
            sound?.pause()
        } else{
            if (!soundEmpty!!){
                val musicButton = findViewById<Button>(R.id.musicButton)
                musicButton.text = "Pause Audio"
                sound?.start()
            }
        }
        if (soundEmpty == true) {
            userLocationPoint = tmpLocation!!
            ...
            markList.forEach {
                var distance: Double = Geo.distance(userLocationPoint, it.position)
                    if (distance < 50) {
                        ...
                        sound?.start()
                        soundEmpty = false
                        val textField = findViewById<TextView>(R.id.markName)
                        textField.text = it.musicName
                        val musicButton = findViewById<Button>(R.id.musicButton)
                        musicButton.text = "Pause Audio"
                    }
